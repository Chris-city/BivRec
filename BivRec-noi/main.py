...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='baby', help='book | taobao')  # 数据集
    ...
    ########## for noi:
    parser.add_argument("--lamda1", default=1, type=float, help="for Recloss")
    parser.add_argument("--lamda2", default=1, type=float, help="for INFONCEloss")
    parser.add_argument("--mm_dim", default=64, type=int)
    parser.add_argument("--noi", default=True, type=bool, help="if noi")
    parser.add_argument("--direct", default=False, type=bool, help="directly mm")
    parser.add_argument("--modality", default='image', type=str, help="image | text | both")
    parser.add_argument("--mm_type", default='concat', type=str, help="no | concat | sum | attention | ")
    ...
    parser.add_argument("--item_num", default=7050, type=int, help="see main.py: 103993sports |23033cloth |3417ml")
    ...
    parser.add_argument("--MLPtoVector", default=True, type=bool,
    ...
    image_file = path + 'image_feat.npy'
    text_file = path + 'text_feat.npy'
    ...
    if args.noi:
        model = MMclus_noi(args)
        lamda1 = args.lamda1
        lamda2 = args.lamda2
        if args.p == 'train':
            train(device=device, train_file=train_file, valid_file=valid_file, test_file=test_file, model=model,
                  batch_size=args.batch_size,
                  seq_len=args.seq_len, hidden_size=args.hidden_size,
                  topN=args.topN, max_iter=args.max_iter, test_iter=args.test_iter, patience=args.patience,
                  exp_name=exp_name, lamda1=lamda1, lamda2=lamda2, image_file=image_file, text_file=text_file)

        elif args.p == 'test':
            test(device=device, test_file=test_file, cate_file=cate_file, model=model, batch_size=args.batch_size,
                 seq_len=args.seq_len,
                 hidden_size=args.hidden_size, topN=args.topN, exp_name=exp_name, coef=args.coef, image_file=image_file, text_file=text_file)
        elif args.p == 'output':
