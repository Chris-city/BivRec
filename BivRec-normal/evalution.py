...
def tsne_plot(model, test_data, device, name):
    for _, (users, targets, items, image_his, text_his, mask) in enumerate(test_data):  # 一个batch的数据2048个用户，每人30个target

        # 获取用户嵌入
        # 多兴趣模型，shape=(batch_size, num_interest, embedding_dim)
        # 其他模型，shape=(batch_size, embedding_dim)
        user_embs, _, _ = model(to_tensor(items, device), None, None, None, to_tensor(mask, device), device,
                                train=False)
        ...
        selected_users_idx = np.random.choice(tsne_batch, size=300, replace=False)
        ...
        np.save(name, selected_users_embs_2d)
        color_map = plt.get_cmap('tab20')  # 20种颜色
        color_idx = 0
        for i, user_idx in enumerate(selected_users_idx):
            ...
            for j in range(num_interests):
                plt.scatter(selected_users_embs_2d[color_idx, 0], selected_users_embs_2d[color_idx, 1],
                            color=user_color, s=2)
                color_idx += 1
...
#         return
    ...
    for _, (users, targets, items, imag_his, image_target, text_his, text_target, mask) in enumerate(
            test_data):  # 一个batch的数据
        # 获取用户嵌入
        # 多兴趣模型，shape=(batch_size, num_interest, embedding_dim)
        # 其他模型，shape=(batch_size, embedding_dim)
        user_embs, _ = model(to_tensor(items, device), None, to_tensor_tupe(imag_his, device), None,
                             to_tensor_tupe(text_his, device), None, to_tensor(mask, device), device, train=False)
...
def train(device, train_file, valid_file, test_file, model, batch_size, seq_len, hidden_size, topN, max_iter, test_iter,
          patience, exp_name, image_file, text_file):
    best_model_path = "best_model/" + exp_name + '/'  # 模型保存路径

    # prepare data
    train_data = get_DataLoader(train_file, batch_size, seq_len, train_flag=1, image_file=image_file,
                                text_file=text_file)
    valid_data = get_DataLoader(valid_file, batch_size, seq_len, train_flag=0, image_file=image_file,
                                text_file=text_file)
    ...
    try:
        ...
        for i, (users, targets, items, image_items, image_target, text_items, text_target, mask) in enumerate(
                train_data):  # 每个批次的数据
            model.train()
            iter += 1
            optimizer.zero_grad()
            _, scores = model(to_tensor(items, device), to_tensor(targets, device), to_tensor_tupe(image_items, device),
                              to_tensor_tupe(image_target, device), to_tensor_tupe(text_items, device),
                              to_tensor_tupe(text_target, device), to_tensor(mask, device), device)
    ...
    # 训练结束后用test_data测试一次
    test_data = get_DataLoader(test_file, batch_size, seq_len, train_flag=0, image_file=image_file, text_file=text_file)
...
def test(device, test_file, cate_file, model, batch_size, seq_len, hidden_size, topN, exp_name, coef=None,
         image_file=None, text_file=None):
