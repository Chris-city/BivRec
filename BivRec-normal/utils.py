...
class DataIterator(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        for user_id in user_id_list:
            ...
            image_target.append(self.image_features[item_id_list[-1]])
            text_target.append(self.text_features[item_id_list[-1]])
        return user_id_list, item_id_list, hist_item_list, image_his_list, image_target, text_his_list, text_target, hist_mask_list


def get_DataLoader(source, batch_size, seq_len, train_flag=1, image_file=None, text_file=None):
    dataIterator = DataIterator(source, batch_size, seq_len, train_flag, image_file, text_file)
...
#     var= torch.tensor([item.cpu().detach().numpy() for item in var]).cuda()
#     var = var.to(device)
#     return var.long()
